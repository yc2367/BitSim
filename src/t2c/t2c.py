...
from src.module.fuse import QConvReLU, QConvBNReLU, MulShift
from src.module.base import _QBaseLinear, IntMatMul, ConvOPS
from src.module.convert import get_parent_name
from src.t2c.resnet import ResNet18Fuser, ResNet34Fuser, ResNet50Fuser
from src.t2c.vgg import VGGFuser
from src.t2c.vit import ViTFuser
from src.t2c.bert import BERTFuser
from src.t2c.mobilenet import MobileNetV1Fuser
...
class T2C(object):
    ...
    def save(self, path:str):
        ...
        total_flops = 0
        flops_dict = {}
        json_path = os.path.join(tensor_dir, "flops.json")
        ...
        for k, v in tqdm(self.node_dict.items()):
            ...
            # # flops
            # # print(f"Module = {k}")
            # flops = self.flops(x1, x2)
            # flops_dict[k] = int(flops.item())
            # total_flops += flops
        ...
        #     json.dump(flops_dict, outfile)

        # print(f"Total FLOPs of MatMul = {int(total_flops)}")
