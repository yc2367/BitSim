...
class Pruner(object):
    ...
    def update_mask(self, threshold):
        for n, m in self.model.named_modules():
            ...
            #     mask = torch.zeros(m.out_features, device=m.weight.device)
            #     scale = m.wq.scale.data.abs().view(-1)
            #     delta = 1 / scale
...
class StructuredPruner(Pruner):
    def __init__(self, model: nn.Module, pr: float = 0.5, grp:int=32):
        super().__init__(model, pr)

        # group size
        self.grp = grp

    def reshape(self, delta:torch.Tensor):
        delta_group = delta.contiguous().view(len(delta)//self.grp, self.grp)
        return delta_group
    ...
    def collect_score(self):
        ...
        for n, m in self.model.named_modules():
            if isinstance(m, _QBase):
                if ".wq" in n:
                    delta = 1 / m.scale.data
                    
                    if len(delta.size()) > 0:
                        if len(delta) % self.grp == 0:
                            delta_group = self.reshape(delta)
                            grp_score = torch.norm(delta_group, p=2, dim=1)
                        
                            channel_scores.append(grp_score.view(-1))
                        else:
                            pass
        ...
        for n, m in self.model.named_modules():
            if isinstance(m, (QConvBNReLU, QConvReLU)):
                ...
                if len(scale) > 1:
                    ...
                    # group mask
                    mask = grp_score.gt(threshold).float()
                    if len(mask.shape) == 1:
                        mask = mask.unsqueeze(1)

                    mask = mask.repeat([1, self.grp])
                ...
                if len(scale) > 1:
                    if len(delta) % self.grp == 0:
                        ...
                        self.masks[n] = (1 - mask).unsqueeze(1)
